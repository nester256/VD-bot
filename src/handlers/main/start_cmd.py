    ...
    try:
        ...
        if access_token is not None:
            ...
            if info['role'] == 'deliverer':
                await send_menu_message_deliverer(message, state)
                return
            addr = info['address']
            if not addr:
                addr = settings.NEW_CUSTOMER_ADDR
            await send_menu_message_customer(message, state, addr)
            return


        user = await create_user_request(
...
async def start_sure_handler(
    callback: types.CallbackQuery, state: FSMContext
) -> None:
    try:
        await callback.answer('')
        await send_menu_message_customer(callback.message, state, settings.NEW_CUSTOMER_ADDR)
        return
    except Exception as err:
